...
delimiters = ["(", ")", "[", "]", ",", "<<", ">>"]
reserved_words = ["feature", "create", "class", "integer","loop", "from", "until", "array", "do", "end", "not", "and", "or", "if", "then", "else"]
operators = ["=", "/=", "<", ">", "<=", ">=", "+", "-","*", "/", "//", "^", ":=", ":", "."]
...
def delimiterRecognizer():
    ...
    if source_code[current_position] in delimiters:
        ...
        if (source_code[current_position]+source_code[current_position+1]) in delimiters:
            lexeme += source_code[current_position]+source_code[current_position+1]
            current_position += 2
        else:
            return False
    else:
...
def throwError():
    ...
    print(f'See description: Word {source_code[current_position]} not recognized.\n')
    current_position+=1
