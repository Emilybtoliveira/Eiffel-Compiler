...
# VERIFICAR TODOS OS ISALNUM
def conflictResolution(non_terminal, terminal, following_tokens):
    if non_terminal == "<identifier_list>":
        # se tem , então <identifier> , <identifier_list>
        ...
        if terminal in ["+", "-"]:
            # se for numero, só pode ser derivado por special_exp -> manifest_constant
            return 1 if following_tokens[0].lexeme_class == 'int' else 0
        # ATENÇÃO: AQUI NAO É O PROXIMO TERMINAL, NECESSARIAMENTE, MAS EM UM DOS PROXIMOS
        if terminal == "INTEGER" or terminal == "<identifier>":
        ...
        # nesse caso aqui, é se tiver OUTRO ponto depois do primeiro
        return 1 if areListsIntersecctioned(["."], following_tokens[1:]) else 0
...
def getFollowingTokens():
    following_tokens = tokens[slice(
        current_token_index+1, current_token_index+6)]
        ...
        try:
           ...
           # print(len(children_list))
            ...
            if (len(children_list) > 1) and (type(children_list[0]) == list):
                children_index = conflictResolution(
                    getTopStack(), terminal, getFollowingTokens())
                # print(children_index)

                pop()
                push(children_list[children_index])
                current_parent_node.addNewChildren(
                    children_list[children_index])
...
def symbolTableConstruct(current, scope, index):
    ...
    for child in current.children:
        ...
        if (child.parent_node.value == "<feature_declaration>"):
            ...
            symbolDc = Simbolo(identifier, typeId,
                               retornotipo, value, "global", 0)
            ...
            symbolDc = Simbolo(idParam, typeParam,
                               returnTypeParam, value, scope, 0)
            SymbolTable.append(symbolDc)
